...
RQ_DEFAULT_TIMEOUT = int(get_env('RQ_DEFAULT_TIMEOUT', 600))
RQ_DATASET_TIMEOUT = int(get_env('RQ_DATASET_TIMEOUT', 1800))
RQ_TRAINING_TIMEOUT = int(get_env('RQ_TRAINING_TIMEOUT', 3600))
...
RQ_QUEUES = {
    'q_datasets': {
        ...
        'PORT': REDIS_PORT,
        'DB': REDIS_DB,
        'DEFAULT_TIMEOUT': RQ_DATASET_TIMEOUT,
    },
    'q_trainings': {
        'HOST': REDIS_HOST,
        'PORT': REDIS_PORT,
        'DB': REDIS_DB,
        'DEFAULT_TIMEOUT': RQ_TRAINING_TIMEOUT,
    },
    'default': {
        'HOST': REDIS_HOST,
        'PORT': REDIS_PORT,
...
# 或自动选择最空闲的 GPU（device=-1）
# 或多个空闲 GPU （device=[-1,-1])
YOLO_TRAIN_DEVICE = get_env('YOLO_TRAIN_DEVICE', 'cpu')
# 批次大小，
# 具有三种模式：
#   设置为整数（例如， batch=16），
#   自动模式，GPU 内存利用率为 60%（batch=-1），
#   或具有指定利用率分数的自动模式（batch=0.70)
YOLO_TRAIN_BATCH = int(get_env('YOLO_TRAIN_BATCH', 16))
# 训练优化器的选择。
# 选项包括 SGD, Adam, AdamW, NAdam, RAdam, RMSProp 等等，或者 auto 用于基于模型配置自动选择。影响收敛速度和稳定性。
YOLO_TRAIN_OPTIMIZER = get_env('YOLO_TRAIN_OPTIMIZER', 'auto')
