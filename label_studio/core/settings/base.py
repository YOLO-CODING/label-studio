...
# training dir
TRAINING_DIR = os.path.join(BASE_DATA_DIR, 'training')
...
PLAN_MIXIN = 'plans.mixins.PlanMixin'
SHIP_DETECTION_MIXIN = 'ship_detections.mixins.ShipDetectionMixin'
LSE_PROJECT = None
...
WEBHOOK_SERIALIZERS = {
    ...
    'plan': 'webhooks.serializers_for_hooks.PlanWebhookSerializer',
}
...
MINIO_DOMAIN_ADDRESS = get_env("MINIO_ADDRESS", "http://localhost:9000")
if get_env('MINIO_STORAGE_ENDPOINT') and not get_bool_env('MINIO_SKIP', False):
    ...
    # AWS_S3_CUSTOM_DOMAIN = MINIO_DOMAIN_ADDRESS.replace('http://', '').replace('https://', '') + '/' + AWS_STORAGE_BUCKET_NAME
    WEB_HOST = get_env('HOST', 'http://localhost:8080')
    AWS_S3_URL_PROTOCOL = 'http:' if WEB_HOST.startswith('http://') else 'https:'
    AWS_S3_CUSTOM_DOMAIN = WEB_HOST.replace('http://', '').replace('https://', '') + '/data'
