...
const webhookActionsMap = {
  'Project updated': '项目更新',
  'Task created': '任务创建',
  'Task deleted': '任务删除',
  'Annotation created': '标注创建',
  'Annotations created': '标注批量创建',
  'Annotation updated': '标注更新',
  'Annotation deleted': '标注删除',
  'Label link created': '标签关联创建',
  'Label link updated': '标签关联更新',
  'Label link deleted': '标签关联删除'
};
    ...
    <Block name="webhook">
      <Elem name="title">
        <>
          ...
          / {webhook === null ? "新建Webhook" : "编辑Webhook"}
        </>
      </Elem>
      <Elem name="content">
            ...
            <Form.Row columnCount={1}>
              ...
              <Space className={rootClass.elem("url-space")}>
                ...
                <Space align="end" className={rootClass.elem("activator")}>
                  <span className={rootClass.elem("black-text")}>启用状态</span>
                  <Toggle
                    skip
                    checked={isActive}
                    onChange={(e) => {
                            ...
                            <Form.Row key={key} columnCount={1}>
                              <div>
                                <Toggle
                                  ...
                                  name={key}
                                  type="checkbox"
                                  label={webhookActionsMap[value.name] ?? value.name}
