...
export const TimeAgo = ({ date, ...rest }: TimeAgoProps) => {
  ...
  const dateStr = formatDistanceToNow(fromTS, { addSuffix: true, locale: zhCN })
  const text = dateStr === "不到 1 分钟前" ? "刚刚": dateStr

  return (
    <time style={{color: "#999", letterSpacing: 0}} dateTime={format(fromTS, "yyyy-MM-dd'T'HH:mm:ss.SSSxxx")} title={format(fromTS, "PPpp")} {...rest}>
      {text}
    </time>
  );
};
