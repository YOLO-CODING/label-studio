...
export const EMPTY_SUBMIT_TOOLTIP = "当前项目需要您贡献有效的标注内容";
    ...
    if (isReview) {
      ...
      if (store.hasInterface("skip")) {
        const onSkipWithComment = (e: React.MouseEvent, action: () => any) => {
          handleActionWithComments(e, action, "请添加备注后跳过");
        };
      ...
      if (userGenerate || (store.explore && !userGenerate && store.hasInterface("submit"))) {
        ...
        const button = (
          <ButtonTooltip key="update" title={noChanges ? "没有未保存的修改" : "更新当前任务: [ Ctrl+Enter ]"}>
            <ButtonGroup>
              <Button
                aria-label="submit"
                name="submit"
