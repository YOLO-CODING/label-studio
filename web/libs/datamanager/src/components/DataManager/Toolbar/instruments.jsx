...
import { TrainingButton,ExportButton, ImportButton } from "../../Common/SDKButtons";
 ...
 */
const ImportButtonWithChecks = ({ size, icon }) => {
  const simpleButton = <ImportButton size={size}>{icon}导入数据</ImportButton>;
  ...
  return (
    ...
    >
      <Block name="button-wrapper">
        <ImportButton disabled size={size}>{icon}导入数据</ImportButton>
      </Block>
    </Tooltip>
  );
};
  ...
  "import-button": ({ size }) => {
    return (
      <Interface name="import">
        <ImportButtonWithChecks size={size} icon={<TbDatabaseImport/>}/>
      </Interface>
    );
  },
  "export-button": ({ size }) => {
    return (
      <Interface name="export">
        <ExportButton size={size}><TbDatabaseExport />导出数据</ExportButton>
      </Interface>
    );
  },
  "training-button": ({ size }) => {
    return (
      <Interface name="training">
        <TrainingButton size={size}><MdModelTraining />启动训练</TrainingButton>
      </Interface>
    );
  },
};
