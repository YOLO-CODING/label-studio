...
export const DataView = injector(
  ({
    ...
    const columnHeaderExtra = useCallback(({ parent, original, help }, decoration) => {
      ...
      if (parent) {
        const key = original?.readableTypeIcon ?? parent.title;
      ...
      if (help && decoration?.help !== false) {
        children.push(
          <Tooltip key="help-tooltip" title={help}>
            <Icon icon={IconQuestionOutline} style={{ opacity: 0.5, cursor: "pointer" }} />
          </Tooltip>,
        );
      }
        ...
        if (
          ...
          return (
            <Block name="syncInProgress">
              ...
              <Elem name="text">请尝试调整筛选条件或搜索参数</Elem>
            </Block>
          );
        }
        if (store.SDK.type === "DE" && (total === 0 || data.length === 0 || !hasData)) {
          return (
            <Block name="syncInProgress">
              ...
              <Elem name="text">点击下方按钮查看已同步的数据</Elem>
              <Button
                size="small"
                look="outlined"
                onClick={async () => {
