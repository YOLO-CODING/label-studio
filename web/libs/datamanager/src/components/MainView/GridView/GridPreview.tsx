...
const TaskModal = observer(({ view, tasks, imageField, currentTaskId, setCurrentTaskId }: TaskModalProps) => {
  ...
  const tooltip = (
    <div className={styles.tooltip}>
      ...
      <p> 任务图片预览功能，支持快速浏览任务列表并勾选待处理任务:</p>
      <p>【方向键】浏览任务列表</p>
      <p>【ESC键】关闭预览窗口</p>
      <p>【空格键】选中/取消选中任务</p>
  ...
  return (
    <div className={styles.modal}>
      ...
      <div className="grid grid-cols-[35px_1fr_35px]" style={{alignItems:'center'}}>
        <FaCircleChevronLeft className={styles.icon} onClick={goToPrev}  style={{opacity: index===0? 0: 1}}/>
          <ImagePreview task={task} field={imageField} />
        <FaCircleChevronRight className={styles.icon}  onClick={goToNext} style={{opacity: index===tasks.length-1? 0: 1}}/>
      </div>
    </div>
  );
});
