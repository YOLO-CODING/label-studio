...
export class LSFWrapper {
  ...
  async preloadTask() {
    ...
    if (params) {
      ...
      const body = `任务 #${taskID}${commentId ? ` 备注 #${commentId}` : ""} 未找到!`;
  ...
  async loadTask(taskID, annotationID, fromHistory = false) {
    ...
    if (isFF(FF_DEV_2887) && this.lsf?.commentStore?.hasUnsaved) {
      ...
      Modal.confirm({
        title: "您有未保存的更改",
        body: "当前存在尚未保存的批注哦。请先提交标注，继续操作将会丢弃批注。",
        onOk() {
          nextAction();
        },
        okText: "放弃并继续",
  ...
  onSubmitAnnotation = async () => {
    ...
    this.showOperationToast(status, "标注信息保存成功", "标注信息未保存", result);
  ...
  onUpdateAnnotation = async (ls, annotation, extraData) => {
    ...
    this.showOperationToast(status, "标注更新成功!", "标注更新失败", result);
  ...
  onSkipTask = async (_, { comment } = {}) => {
    ...
    this.showOperationToast(status, "跳过任务成功", "任务未跳过", result);
  };
