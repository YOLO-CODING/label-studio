...
# Download Cypress binary first
RUN wget -q https://cdn.npmmirror.com/binaries/cypress/14.5.0/linux-x64/cypress.zip -O /tmp/cypress.zip && \
    mkdir -p /root/.cache/Cypress && \
    unzip /tmp/cypress.zip -d /root/.cache/Cypress/14.5.0 && \
    rm /tmp/cypress.zip
...
RUN set -eux; \
    ...
    chown -R 1000:1000 $LS_DIR $LABEL_STUDIO_BASE_DATA_DIR $OPT_DIR /var/log/nginx /etc/nginx

COPY --chown=1000:1000 deploy/default.conf /etc/nginx/nginx.conf
...
COPY --chown=1000:1000 LICENSE LICENSE
COPY --chown=1000:1000 licenses licenses
COPY --chown=1000:1000 deploy deploy

# Copy files from build stages
COPY --chown=1000:1000 --from=venv-builder               $LS_DIR                                           $LS_DIR
COPY --chown=1000:1000 --from=py-version-generator       $LS_DIR/label_studio/core/version_.py             $LS_DIR/label_studio/core/version_.py
COPY --chown=1000:1000 --from=frontend-builder           $LS_DIR/web/dist                                  $LS_DIR/web/dist
COPY --chown=1000:1000 --from=frontend-version-generator $LS_DIR/web/dist/apps/labelstudio/version.json    $LS_DIR/web/dist/apps/labelstudio/version.json
COPY --chown=1000:1000 --from=frontend-version-generator $LS_DIR/web/dist/libs/editor/version.json         $LS_DIR/web/dist/libs/editor/version.json
COPY --chown=1000:1000 --from=frontend-version-generator $LS_DIR/web/dist/libs/datamanager/version.json    $LS_DIR/web/dist/libs/datamanager/version.json
